...
void sh_step() {
    ...
    if(myHandler.currentIndex <=99){
    	myHandler.currentIndex +=1;
    }
    else{
    	myHandler.currentIndex = 1;
    }
    myHandler.instructionCounter = myHandler.currentIndex;
}
...
int printAll() {
    ...
    mt_gotoXY(5, 71);
    mt_setForeground(LIGHT_BLUE);
    std::cout << '+' << std::dec << std::setw(4) << std::setfill('0') << myHandler.instructionCounter << std::dec;
    mt_setForeground(BLUE);
    mt_gotoXY(7, 68);
    std::cout << " Operation ";
    mt_gotoXY(10, 70);
    ...
    while(key != closeApp && tempFlag == 1) {
        ...
        switch(key) {
            ...
            case KEYS::instructionCounter:
                ...
                std::cin >> value1;
                if(value1<=100 && value1 >=1)
                	myHandler.currentIndex = value1-1;
                myHandler.instructionCounter = myHandler.currentIndex;
                value &= 0b111111111111111;
                std::cin >> std::dec;
                mt_setForeground(BLUE);
